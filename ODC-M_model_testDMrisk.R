...
Non_CVD_DM_mortality <- fread("00 Input Data/Non_CVD_Non_DM_Cause_Mortality (Annual probability).csv", stringsAsFactors = TRUE, data.table = FALSE)
...
stroke_mortality <- fread("00 Input Data/Stroke_Cause_Mortality (Annual probability).csv", stringsAsFactors = TRUE, data.table = FALSE)
...
DM_mortality <- fread("00 Input Data/DM_Cause_Mortality (Annual probability).csv", stringsAsFactors = TRUE, data.table = FALSE)
...
risk_factor_trend <- fread("00 Input Data/Risk_Factors_Trends_Summary (Final).csv", stringsAsFactors = TRUE, data.table = FALSE)
...
run_sim <- function(s) {
  ...
  sim_out_filter <- array(NA, dim=c(n.individual, length(all_variables), n.cycle+1),
                          dimnames = list(data_for_analysis$Subject_ID,
                                          all_variables, paste("cycle", 0:n.cycle, sep = " ")))
  ...
  sim_out[,"cost_disc",1] <- data_for_analysis$HCE_predict
...
#  sim_out_filter[,,1] <- sim_out[,,1]
 # sim_out_filter[as.numeric(sim_out[,"Age_cycle",1])<40 | as.numeric(sim_out[,"Age_cycle",1])>79,,1] = NA
  ...
  for (t in 1:n.cycle) {
    ...
    sim_out[,"ssb",t+1] <- as.numeric(sim_out[,"ssb",1])*(1-policy_effect_ssb[t,s])
    sim_out[,"added_sugar",t+1] <- as.numeric(sim_out[,"added_sugar",1])*(1-policy_effect_added_sugar[t,s])
    sim_out[,"sodium",t+1] <- as.numeric(sim_out[,"sodium",1])*(1-policy_effect_sodium[t,s])
    sim_out[,"kcal",t+1] <- as.numeric(sim_out[,"kcal",1])*(1-policy_effect_kcal[t,s])
    sim_out[,"sfat",t+1] <- as.numeric(sim_out[,"sfat",1])*(1-policy_effect_sfat[t,s])
    ...
    sim_out[,"BMI",t+1] <- ifelse(as.numeric(sim_out[,"BMI",t+1]) <25,
                                  as.numeric(sim_out[,"BMI",t+1])+random_beta_Sugar_BMI_Low[1,s]*(as.numeric(sim_out[,"added_sugar",t+1]) - as.numeric(sim_out[,"added_sugar",1]))+random_beta_SSB_BMI_Low[1,s]*(as.numeric(sim_out[,"ssb",t+1]) - as.numeric(sim_out[,"ssb",1])),
                                  as.numeric(sim_out[,"BMI",t+1])+random_beta_Sugar_BMI_high[1,s]*(as.numeric(sim_out[,"added_sugar",t+1]) - as.numeric(sim_out[,"added_sugar",1]))+random_beta_SSB_BMI_high[1,s]*(as.numeric(sim_out[,"ssb",t+1]) - as.numeric(sim_out[,"ssb",1])))
    ...
    sim_out[,"SBP",t+1] <- case_when(
      as.numeric(sim_out[,"Age_cycle",t]) < 45 ~ as.numeric(sim_out[,"SBP",t+1])+random_beta_Na_SBP_main[1,s]*((as.numeric(sim_out[,"sodium",t+1]) - as.numeric(sim_out[,"sodium",1]))/2.3),
      as.numeric(sim_out[,"Age_cycle",t]) < 55 ~ as.numeric(sim_out[,"SBP",t+1])+random_beta_Na_SBP_main[2,s]*((as.numeric(sim_out[,"sodium",t+1]) - as.numeric(sim_out[,"sodium",1]))/2.3),
      as.numeric(sim_out[,"Age_cycle",t]) < 65 ~ as.numeric(sim_out[,"SBP",t+1])+random_beta_Na_SBP_main[3,s]*((as.numeric(sim_out[,"sodium",t+1]) - as.numeric(sim_out[,"sodium",1]))/2.3),
      as.numeric(sim_out[,"Age_cycle",t]) < 75 ~ as.numeric(sim_out[,"SBP",t+1])+random_beta_Na_SBP_main[4,s]*((as.numeric(sim_out[,"sodium",t+1]) - as.numeric(sim_out[,"sodium",1]))/2.3),
    ...
    p.transition[,"Subsequent CHD w/o RVSC"] <- ifelse(sim_out[,"state",t] == "CVD History, With Diabetes", (1-p.RVSC)*p.CVD.2.Sub_CHD, 
                                                       ifelse(sim_out[,"state",t] == "CVD History, No Diabetes", (1-p.RVSC)*p.CVD_DM.2.Sub_CHD, rep(0, n.individual)))
    p.transition[,"Subsequent CHD with RVSC"] <- ifelse(sim_out[,"state",t] == "CVD History, With Diabetes", p.RVSC*p.CVD.2.Sub_CHD, 
   ...
   # set.seed(seed)
    ...
    raw.input.data  <- cbind(as.numeric(sim_out[,"seqn",t+1]), as.numeric(sim_out[,"Age_cycle",t+1]), as.numeric(sim_out[,"Female",t+1]), as.numeric(sim_out[,"Race",t+1]),
                             as.numeric(sim_out[,"HPT_Txt",t+1]), as.numeric(sim_out[,"BMI",t+1]), 
                             as.numeric(sim_out[,"Diabetes",t+1]),  as.numeric(sim_out[,"CVD_history",t+1]))
    colnames(raw.input.data) <- c("seqn","Age","Female","Race","HPT_Txt", "BMI","Diabetes","CVD_history")
    raw.input.data <- as.data.frame(raw.input.data)
...
m.state <- array(sim_out[,"state",1:n.cycle,], dim=c(n.individual, n.cycle, n.sim),
                 dimnames = list(paste("ind", 1:n.individual, sep = " "), paste("cycle", 1:n.cycle, sep = " "), paste("simulation_", 1:n.sim, sep = " ")))
m.effect <- array(as.numeric(sim_out[,"HRQOL_scores",1:n.cycle,]), dim=c(n.individual, n.cycle, n.sim),
                  dimnames = list(paste("ind", 1:n.individual, sep = " "), paste("cycle", 1:n.cycle, sep = " "), paste("simulation_", 1:n.sim, sep = " ")))
m.cost <- array(as.numeric(sim_out[,"HCE_predict",1:n.cycle,]), dim=c(n.individual, n.cycle, n.sim),
                dimnames = list(paste("ind", 1:n.individual, sep = " "), paste("cycle", 1:n.cycle, sep = " "), paste("simulation_", 1:n.sim, sep = " ")))
m.effect_disc <- array(as.numeric(sim_out[,"effect_disc",1:n.cycle,]), dim=c(n.individual, n.cycle, n.sim),
                       dimnames = list(paste("ind", 1:n.individual, sep = " "), paste("cycle", 1:n.cycle, sep = " "), paste("simulation_", 1:n.sim, sep = " ")))
m.cost_disc <- array(as.numeric(sim_out[,"cost_disc",1:n.cycle,]), dim=c(n.individual, n.cycle, n.sim),
                     dimnames = list(paste("ind", 1:n.individual, sep = " "), paste("cycle", 1:n.cycle, sep = " "), paste("simulation_", 1:n.sim, sep = " ")))
...
for (race in races) {
  ...
  mean_out <- array(NA, dim = c(length(vars), n.cycle, n.sim), 
                    dimnames = list(vars, paste("cycle", 1:n.cycle, sep = " "), paste("simulation_", 1:n.sim, sep = " ")))
  within_var_out <- array(NA, dim = c(length(vars), n.cycle, n.sim), 
                          dimnames = list(vars, paste("cycle", 1:n.cycle, sep = " "), paste("simulation_", 1:n.sim, sep = " ")))
  ...
  for (t in 1:n.cycle) {
    ...
    if (race == "all") {
      mean_out["Obesity",t,] <- svymean(sim_out[,"Obesity",t,]==1, design, deff=F, na.rm=T)
      within_var_out["Obesity",t,] <- SE(svymean(sim_out[,"Obesity",t,]==1, design, deff=F, na.rm=T))^2
      mean_out["Diabetes",t,] <- svymean(sim_out[,"Diabetes",t,]==1, design, deff=F, na.rm=T)
      within_var_out["Diabetes",t,] <- SE(svymean(sim_out[,"Diabetes",t,]==1, design, deff=F, na.rm=T))^2
      mean_out["CVD_history",t,] <- svymean(sim_out[,"CVD_history",t,]==1, design, deff=F, na.rm=T)
      within_var_out["CVD_history",t,] <- SE(svymean(sim_out[,"CVD_history",t,]==1, design, deff=F, na.rm=T))^2
      mean_out["All-cause mortality",t,] <- svymean(m.state[,t,]=="Death", design, deff=F, na.rm=T)
      within_var_out["All-cause mortality",t,] <- SE(svymean(m.state[,t,]=="Death", design, deff=F, na.rm=T))^2
      mean_out["Incident Stroke",t,] <- svymean(m.state[,t,]=="First Stroke", design, deff=F, na.rm=T)
      within_var_out["Incident Stroke",t,] <- SE(svymean(m.state[,t,]=="First Stroke", design, deff=F, na.rm=T))^2
      mean_out["Incident CHD",t,] <- svymean(m.state[,t,]=="First CHD with RVSC" | m.state[,t,]=="First CHD w/o RVSC", design, deff=F, na.rm=T)
      within_var_out["Incident CHD",t,] <- SE(svymean(m.state[,t,]=="First CHD with RVSC" | m.state[,t,]=="First CHD w/o RVSC", design, deff=F, na.rm=T))^2
      mean_out["QALY",t,] <- svymean(m.effect[,t,], design, deff=F, na.rm=T)
      within_var_out["QALY",t,] <- SE(svymean(m.effect[,t,], design, deff=F, na.rm=T))^2
      mean_out["QALY_disc",t,] <- svymean(m.effect_disc[,t,], design, deff=F, na.rm=T)
      within_var_out["QALY_disc",t,] <- SE(svymean(m.effect_disc[,t,], design, deff=F, na.rm=T))^2
      mean_out["HCE",t,] <- svymean(m.cost[,t,], design, deff=F, na.rm=T)
      within_var_out["HCE",t,] <- SE(svymean(m.cost[,t,], design, deff=F, na.rm=T))^2
      mean_out["cost_disc",t,] <- svymean(m.cost_disc[,t,], design, deff=F, na.rm=T)
      within_var_out["cost_disc",t,] <- SE(svymean(m.cost_disc[,t,], design, deff=F, na.rm=T))^2
    } else {
      mean_out["Obesity",t,] <- svymean(sim_out[which(data_for_analysis$Race==race_num),"Obesity",t,]==1, race_design, deff=F, na.rm=T)
      within_var_out["Obesity",t,] <- SE(svymean(sim_out[which(data_for_analysis$Race==race_num),"Obesity",t,]==1, race_design, deff=F, na.rm=T))^2
      mean_out["Diabetes",t,] <- svymean(sim_out[which(data_for_analysis$Race==race_num),"Diabetes",t,]==1, race_design, deff=F, na.rm=T)
      within_var_out["Diabetes",t,] <- SE(svymean(sim_out[which(data_for_analysis$Race==race_num),"Diabetes",t,]==1, race_design, deff=F, na.rm=T))^2
      mean_out["CVD_history",t,] <- svymean(sim_out[which(data_for_analysis$Race==race_num),"CVD_history",t,]==1, race_design, deff=F, na.rm=T)
      within_var_out["CVD_history",t,] <- SE(svymean(sim_out[which(data_for_analysis$Race==race_num),"CVD_history",t,]==1, race_design, deff=F, na.rm=T))^2
      mean_out["All-cause mortality",t,] <- svymean(m.state[which(data_for_analysis$Race==race_num),t,]=="Death", race_design, deff=F, na.rm=T)
      within_var_out["All-cause mortality",t,] <- SE(svymean(m.state[which(data_for_analysis$Race==race_num),t,]=="Death", race_design, deff=F, na.rm=T))^2
      mean_out["Incident Stroke",t,] <- svymean(m.state[which(data_for_analysis$Race==race_num),t,]=="First Stroke", race_design, deff=F, na.rm=T)
      within_var_out["Incident Stroke",t,] <- SE(svymean(m.state[which(data_for_analysis$Race==race_num),t,]=="First Stroke", race_design, deff=F, na.rm=T))^2
      mean_out["Incident CHD",t,] <- svymean(m.state[which(data_for_analysis$Race==race_num),t,]=="First CHD with RVSC" | 
                                               m.state[which(data_for_analysis$Race==race_num),t,]=="First CHD w/o RVSC", race_design, deff=F, na.rm=T)
      within_var_out["Incident CHD",t,] <- SE(svymean(m.state[which(data_for_analysis$Race==race_num),t,]=="First CHD with RVSC" | 
                                                        m.state[which(data_for_analysis$Race==race_num),t,]=="First CHD w/o RVSC", race_design, deff=F, na.rm=T))^2
      mean_out["QALY",t,] <- svymean(m.effect[which(data_for_analysis$Race==race_num),t,], race_design, deff=F, na.rm=T)
      within_var_out["QALY",t,] <- SE(svymean(m.effect[which(data_for_analysis$Race==race_num),t,], race_design, deff=F, na.rm=T))^2
      mean_out["QALY_disc",t,] <- svymean(m.effect_disc[which(data_for_analysis$Race==race_num),t,], race_design, deff=F, na.rm=T)
      within_var_out["QALY_disc",t,] <- SE(svymean(m.effect_disc[which(data_for_analysis$Race==race_num),t,], race_design, deff=F, na.rm=T))^2
      mean_out["HCE",t,] <- svymean(m.cost[which(data_for_analysis$Race==race_num),t,], race_design, deff=F, na.rm=T)
      within_var_out["HCE",t,] <- SE(svymean(m.cost[which(data_for_analysis$Race==race_num),t,], race_design, deff=F, na.rm=T))^2
      mean_out["cost_disc",t,] <- svymean(m.cost_disc[which(data_for_analysis$Race==race_num),t,], race_design, deff=F, na.rm=T)
      within_var_out["cost_disc",t,] <- SE(svymean(m.cost_disc[which(data_for_analysis$Race==race_num),t,], race_design, deff=F, na.rm=T))^2
    }
  }
  saveRDS(mean_out, file = paste("40 Results/means", race, intervention, "SEED", seed, Sys.Date(), ".rda", sep = "_"))
  saveRDS(within_var_out, file = paste("40 Results/vars", race, intervention, "SEED", seed, Sys.Date(), ".rda", sep = "_"))
}
